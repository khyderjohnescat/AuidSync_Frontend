    ...
    return (
        <div className="p-6 text-white">
            <div className="flex justify-between items-center">
                ...
                <Link to="/manageproduct" className="bg-blue-500 px-4 py-2 rounded">Back to Products</Link>
            </div>
            ...
            {filteredProducts.length === 0 ? (
                <p className="text-gray-400 text-center mt-4">No deleted products found.</p>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {filteredProducts.map((product) => {
                        const imageUrl = product.image
                            ? (product.image.startsWith("http") ? product.image : `https://your-api-url.com/uploads/${product.image}`)
